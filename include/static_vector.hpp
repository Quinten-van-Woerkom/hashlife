...
public:
  ...
  static_vector(std::size_t count, Args &&... args)
      : _storage{new T[count]}, _size{count} {
    fill(T{args...});
  }

  static_vector(const static_vector &other)
      : _storage{new T[other.size()]}, _size{other.size()} {
    std::copy(other.begin(), other.end(), begin());
  }

  constexpr auto operator=(static_vector &&) noexcept
  ...
  constexpr auto rbegin() noexcept {
    return std::rbegin(_storage.get() + _size);
  }
  ...
  constexpr auto crbegin() const noexcept {
    return std::crbegin(_storage.get() + _size);
  }
 ...
 */
template <typename T>
auto static_vector<T>::operator=(const static_vector &other)
    -> static_vector & {
  if (size() != other.size()) {
 ...
 */
template <typename T>
constexpr auto static_vector<T>::operator[](std::size_t index) const
    -> const T & {
  assert(index < size() && "static_vector: Index out of bounds");
  return _storage[index];
}