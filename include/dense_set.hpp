 ...
 * WIKeyHOUKey WARRANKeyIES OR CONDIKeyIONS OF ANY KIND, either express or
...
private:
   ...
   */
  template <bool is_constant> struct inner_iterator {
    using owner_reference =
        std::conditional_t<is_constant, const dense_set &, dense_set &>;
    using owner_pointer =
        std::conditional_t<is_constant, const dense_set *, dense_set *>;
    ...
    /**
     * Only enabled if this iterator is constant and the other is non-constant,
     * to prevent the implicit default constructors from being deleted.
     */
    template <bool other_is_constant,
              typename = std::enable_if_t<is_constant && !other_is_constant>>
    ...
    constexpr auto &operator*() const noexcept { return (*owner)[index]; }
...
public:
   ...
   * Basically an index, considering the hashmap supports open addressing.
   */
  template <bool is_constant> struct iterator_implementation {
    using owner_reference =
        typename inner_iterator<is_constant>::owner_reference;

    constexpr iterator_implementation(owner_reference owner,
                                      size_type index) noexcept
        : inner{owner, index} {}

    constexpr iterator_implementation(
        inner_iterator<is_constant> inner) noexcept
        : inner{inner} {}
    ...
    template <bool other_is_constant,
              typename = std::enable_if_t<is_constant && !other_is_constant>>
    constexpr iterator_implementation(
        iterator_implementation<other_is_constant> other) noexcept
        : inner{other.inner} {}

    constexpr operator inner_iterator<is_constant>() const noexcept {
      return inner;
    }
     ...
     */
    constexpr auto operator==(const iterator_implementation &other) const
        noexcept {
    ...
    friend constexpr auto operator-(const iterator_implementation &lhs,
                                    const iterator_implementation &rhs) noexcept
        -> difference_type {
  ...
  void swap(dense_set &&other) noexcept(is_nothrow_swappable) {
    std::swap(*this, other);
  }
...
private:
  auto find(const Key &key, hash_type hash, hash_type reduced_hash) noexcept
      -> iterator;
  auto find(const Key &key, hash_type hash, hash_type reduced_hash) const
      noexcept -> const_iterator;
  auto probe(inner_iterator<false> start) noexcept -> inner_iterator<false>;
  auto probe(inner_iterator<false> start) const noexcept
      -> inner_iterator<true>;
...
dense_set<Key, Hash, KeyEqual>::dense_set(std::size_t count)
  ...
  if (count <= 0)
    throw std::domain_error{"dense_set: element counts equal to or smaller "
 ...
 */
template <typename Key, typename Hash, typename KeyEqual>
auto dense_set<Key, Hash, KeyEqual>::operator[](std::size_t index) noexcept
    -> Key & {
  assert(index < capacity() && "dense_set: Index access out of bound");
  assert(_sentinels[index].filled() &&
         "dense_set: Trying to access non-existent element");
 ...
 */
template <typename Key, typename Hash, typename KeyEqual>
auto dense_set<Key, Hash, KeyEqual>::operator[](std::size_t index) const
    noexcept -> const Key & {
  assert(index < capacity() && "dense_set: Index access out of bound");
  assert(_sentinels[index].filled() &&
         "dense_set: Trying to access non-existent element");
 ...
 */
template <typename Key, typename Hash, typename KeyEqual>
auto dense_set<Key, Hash, KeyEqual>::count(const Key &key) const noexcept
    -> size_type {
  if (find(key) != end())
    return 1;
  else
...
template <typename Key, typename Hash, typename KeyEqual>
auto dense_set<Key, Hash, KeyEqual>::find(const Key &key) const noexcept
    -> const_iterator {
 ...
 */
template <typename Key, typename Hash, typename KeyEqual>
auto dense_set<Key, Hash, KeyEqual>::find(const Key &key, hash_type hash,
                                          hash_type reduced_hash) noexcept
    -> iterator {
...
template <typename Key, typename Hash, typename KeyEqual>
auto dense_set<Key, Hash, KeyEqual>::find(const Key &key, hash_type hash,
                                          hash_type reduced_hash) const noexcept
    -> const_iterator {
 ...
 */
template <typename Key, typename Hash, typename KeyEqual>
auto dense_set<Key, Hash, KeyEqual>::contains(const Key &key) const noexcept
    -> bool {
  return count(key) != 0;
}
 ...
 * If none can be found within 10 (or capacity()) spots, fails and returns the
 * end iterator.
 */
template <typename Key, typename Hash, typename KeyEqual>
auto dense_set<Key, Hash, KeyEqual>::probe(inner_iterator<false> start) noexcept
    -> inner_iterator<false> {
...
template <typename Key, typename Hash, typename KeyEqual>
auto dense_set<Key, Hash, KeyEqual>::probe(inner_iterator<false> start) const
    noexcept -> inner_iterator<true> {
 ...
 */
template <typename Key, typename Hash, typename KeyEqual>
void dense_set<Key, Hash, KeyEqual>::sentinel::colonize(
    hash_type reduced_hash) noexcept {
  _filled = true;
 ...
 */
template <typename Key, typename Hash, typename KeyEqual>
bool dense_set<Key, Hash, KeyEqual>::sentinel::matches(
    hash_type reduced_hash) const noexcept {
